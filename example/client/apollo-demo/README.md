...
Actually, DependencyManager supports two object to ask which queris should be refetched
```ts

/*
 * Deeply compare old object and new object, if there are some changes, find the operation names of affected queries
 *
 * 1. fetcher is used to specify a comparison boundary, it won't compare deeper asociated object outside of this boundary
 * 2. fetcher does not accept empty fetcher/empty sub fetcheres, it must accept fetcher with fields
 * 3. 2. Actually, the "result.dependencies.ofResult()" in the chapter about typed mutation hook calls this function
 */
...
/*
 * Don't care the data, unconditionlly find the operation names of queries whose fetcher boundary has intersect with the speicified fetcher
 *
