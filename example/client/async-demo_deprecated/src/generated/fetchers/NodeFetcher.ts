...
export interface NodeFetcher<T extends object> extends Fetcher<'Node', T> {
	...
	on<XName extends ImplementationType<'Node'>, X extends object>(child: Fetcher<XName, X>): NodeFetcher<
		XName extends 'Node' ?
		T & X :
	...
	asFragment(name: string): Fetcher<'Node', T>;

	readonly id: NodeFetcher<T & {readonly id: number}>;
