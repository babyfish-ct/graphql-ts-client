...
import { loadTypedQuery, OperationVariablesOf, useTypedQueryLoader } from "../__generated";
...
export const Demo: FC = memo(() => {
    ...
    const [variables, setVariables] = useState<OperationVariablesOf<typeof DEMO1_EMPLOYEE_LIST_QUERY>>({ includeDepartment: false});

    const onNameChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        const newVariables: OperationVariablesOf<typeof DEMO1_EMPLOYEE_LIST_QUERY> = {...variables, name: e.target.value};
    ...
    const onIncludeDepartentChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        const newVariables: OperationVariablesOf<typeof DEMO1_EMPLOYEE_LIST_QUERY> = {...variables, includeDepartment: e.target.checked};
        setVariables(newVariables);
        loadQuery(
            newVariables,
            { fetchPolicy: "network-only" }
